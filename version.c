  ...
  if (tt[0] == '\0')
    {
      if (release_status)
#if HAVE_SNPRINTF
	snprintf (tt, sizeof (tt), "%s.%d(%d)-%s-hardened", dist_version, patch_level, build_version, release_status);
#else
	sprintf (tt, "%s.%d(%d)-%s-hardened", dist_version, patch_level, build_version, release_status);
#endif
      else
#if HAVE_SNPRINTF
	snprintf (tt, sizeof (tt), "%s.%d(%d)-hardened", dist_version, patch_level, build_version);
#else
	sprintf (tt, "%s.%d(%d)-hardened", dist_version, patch_level, build_version);
#endif
    }
  return tt;
}
...
{
  printf (_("GNU bash (HARDENED SINGLE-THREADED ALLOCATOR GUARD), version %s (%s)\n"), shell_version_string (), MACHTYPE);
  if (extended)
    {
      printf ("%s\n", _(bash_copyright));
      printf ("%s\n", _(bash_license));
